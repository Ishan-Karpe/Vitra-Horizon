...
class MockAIService {
  ...
  async getAdvancedPrediction(parameters, userData, goalsData) {
    ...
    const proteinMultiplier =
      proteinIntake === "High" ? 1.2 : proteinIntake === "Medium" ? 1.0 : 0.8;
    ...
    const effectiveness =
      baseEffectiveness *
      ageMultiplier *
      (1 - Math.abs(baseEffectiveness - 1) * 0.1);
    ...
    const bodyFatReduction =
      bodyFatDifference * effectiveness * difficultyMultiplier;
    ...
    const weeklyMuscleGain =
      userWeight * weeklyRate * trainingEffect * nutritionEffect;
    const muscleGain = weeklyMuscleGain * timelineWeeks;

    // Confidence calculation
    const dataQuality = 0.8; // Assume good data quality
    const planComplexity = Math.min(
      1,
      (exerciseFrequency + calorieDeficit / 100) / 8,
    );
    ...
    const adherenceScore = Math.max(
      0.4,
      1 - (exerciseFrequency - 4) * 0.1 - (calorieDeficit - 300) * 0.0005,
    );
    const successProbability = Math.min(95, confidence * adherenceScore);

    // Risk factors
    const riskFactors = [];
    if (exerciseFrequency > 6)
      riskFactors.push("High exercise frequency may lead to overtraining");
    if (calorieDeficit > 500)
      riskFactors.push("Large calorie deficit may affect muscle retention");
    if (bodyFatDifference > 8)
      riskFactors.push(
        "Aggressive body fat target may be difficult to maintain",
      );
    ...
    return [
      {
        scenario: {
          ...
          prediction: await this.getAdvancedPrediction(baseScenario, userData, {
            targetBodyFat: 21,
            timelineWeeks: 12,
          }),
        ...
        rationale:
          "Balanced approach optimizing for both effectiveness and sustainability based on your profile.",
      ...
      },
      {
        scenario: {
          ...
          prediction: await this.getAdvancedPrediction(
            {
              exerciseFrequency: 5,
              calorieDeficit: 250,
              proteinIntake: "High",
            ...
            userData,
            { targetBodyFat: 21, timelineWeeks: 12 },
          ),
          isFromOnboarding: false,
          isFavorite: false,
        },
        rationale:
          "Higher frequency, lower deficit approach for better muscle retention.",
...
class RealAIService extends MockAIService {
  constructor() {
    ...
    console.log(
      `   OpenRouter GLM-4.5: ${this.openrouterApiKey ? "Configured ✅" : "Not configured ❌"}`,
    );
    console.log(
      `   Claude: ${this.claudeApiKey ? "Configured ✅" : "Not configured ❌"}`,
    );
  }
  ...
  async callOpenRouterAPI(prompt) {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        console.error(
          `🕐 OpenRouter API call timed out after ${responseTime}ms`,
        );
  ...
  async getAdvancedPrediction(parameters, userData, goalsData) {
    console.log(
      "🔮 Attempting prediction with OpenRouter GLM-4.5 (Primary)...",
    );
    console.log(
      "🔑 OpenRouter API Key status:",
      this.openrouterApiKey
        ? `Available (${this.openrouterApiKey.length} chars)`
        : "Not available",
    );
    ...
    try {
      ...
      if (
        this.openrouterApiKey &&
        this.openrouterApiKey !== "your_openrouter_key_here"
      ) {
...
Use realistic fitness science principles and provide accurate predictions.`;
        ...
        try {
          ...
          console.warn(
            "Failed to parse OpenRouter JSON response, using enhanced mock",
          );
        }

        // Fallback to enhanced mock with OpenRouter branding
        const prediction = await super.getAdvancedPrediction(
          parameters,
          userData,
          goalsData,
        );
      ...
      console.warn(
        "⚠️ OpenRouter prediction failed, trying Claude backup:",
        error.message,
      );
...
Provide a JSON response with realistic fitness predictions including confidence intervals, success probability, risk factors, plateau prediction, and metabolic adaptation data.`;
          ...
          try {
            ...
            console.warn(
              "Failed to parse Claude JSON response, using enhanced mock",
            );
          }
        ...
        console.warn(
          "⚠️ Claude backup also failed, using mock prediction:",
          claudeError.message,
        );
  ...
  async getChatResponse(message, context) {
    ...
    try {
      ...
      if (
        ...
        const prompt = `Limit your response to 4 sentences maximum. You are a knowledgeable fitness AI assistant. Answer this fitness question: "${message}"

Context: ${JSON.stringify(context)}

Provide a helpful, accurate, and personalized response based on fitness science. Keep it conversational and practical. Limit your response to 4 sentences maximum.`;
      ...
      console.warn(
        "⚠️ OpenRouter chat failed, trying Claude backup:",
        error.message,
      );
...
Provide a helpful, accurate, and personalized response based on fitness science. Keep it conversational and practical. Limit your response to 4 sentences maximum.`;
        ...
        console.warn(
          "⚠️ Claude backup also failed, using mock response:",
          claudeError.message,
        );
...
app.get("/api/health", (req, res) => {
  ...
  res.json({
    ...
    services: {
      "openrouter-glm-4.5": openrouterConfigured
        ? "configured"
        : "not-configured",
...
app.post("/api/predictions", async (req, res) => {
  ...
  try {
    ...
    // Generate prediction
    const prediction = await aiService.getAdvancedPrediction(
      parameters,
      userData,
      goalsData,
    );
...
const server = app.listen(PORT, () => {
  console.log(
    `🚀 Vitra-Morph AI Backend Server running on http://localhost:${PORT}`,
  );
  console.log(`📊 Health check: http://localhost:${PORT}/api/health`);
  console.log(
    `🤖 AI Predictions: POST http://localhost:${PORT}/api/predictions`,
  );
  console.log(`💬 AI Chat: POST http://localhost:${PORT}/api/chat`);
  console.log(
    `🎯 Scenario Generation: POST http://localhost:${PORT}/api/scenarios/generate`,
  );
  console.log(`🔄 Server is running and waiting for connections...`);
  console.log(
    `🧠 AI Strategy: OpenRouter GLM-4.5 (Primary) → Claude 3.5 Sonnet (Backup) → Mock AI (Fallback)`,
  );
