    ...
    <ScrollView className="flex-1 bg-gray-50 p-6">
      ...
      <View className="bg-white rounded-lg p-6 mb-6 shadow-sm">
        ...
        <View className="flex-row items-center mb-3">
          ...
          />
          <Text className="text-gray-700 font-medium">{getStatusText()}</Text>
        </View>

        <Text className="text-sm text-gray-500 mb-3">AI Mode: {aiMode}</Text>
          ...
          {(["offline", "ai-enhanced", "ai-only"] as const).map((mode) => (
            <TouchableOpacity
              ...
              className={`px-3 py-2 rounded-lg ${
                aiMode === mode ? "bg-blue-500" : "bg-gray-200"
              }`}
            >
              <Text
        ...
        {lastPrediction && (
          <View className="mt-4 p-4 bg-gray-50 rounded-lg">
            ...
            <Text className="text-sm text-gray-700">
              Body Fat: {lastPrediction.currentBodyFat}% →{" "}
              {lastPrediction.targetBodyFat}%
            </Text>
            <Text className="text-sm text-gray-700">
      ...
      <View className="bg-white rounded-lg p-6 shadow-sm">
        ...
        <Text className="text-sm text-gray-700 mb-2">
          <Text className="font-medium">Backend URL:</Text>{" "}
          http://localhost:8087
        </Text>
        <Text className="text-sm text-gray-700 mb-2">
        ...
        <Text className="text-sm text-gray-700 mb-2">
          <Text className="font-medium">AI Features:</Text> Predictions, Chat,
          Scenario Generation
        </Text>
        <Text className="text-sm text-gray-700">
          <Text className="font-medium">Fallback:</Text> Simple math when AI
          unavailable
        </Text>
      </View>
    </ScrollView>
