...
export const ComparisonCards: React.FC<ComparisonCardsProps> = ({
  ...
  const getImprovementIndicator = (
    ...
    if (Math.abs(diff) < 0.1)
      return { icon: "→", color: "text-gray-500", text: "Same" };
    if (isImprovement)
      return { icon: "↗️", color: "text-green-500", text: "Better" };
  ...
  const PlanCard: React.FC<{
    ...
    <View
      className={`flex-1 p-4 rounded-xl ${isNew ? "bg-blue-50 border border-blue-200" : "bg-gray-50"}`}
    >
      <Text
        className={`text-sm font-medium mb-3 ${isNew ? "text-blue-600" : "text-gray-600"}`}
      >
        {title}
      </Text>
      ...
      <View className="mb-4">
        <Text
          className={`text-3xl font-bold ${isNew ? "text-blue-600" : "text-gray-900"}`}
        >
          {Number(prediction.bodyFatPercentage).toFixed(1)}%
        </Text>
        <Text
          className={`text-sm ${isNew ? "text-blue-500" : "text-gray-500"}`}
        >
          body fat
        </Text>
      </View>
      ...
      <View className="space-y-2">
        ...
        <View className="flex-row items-center">
          ...
          {isNew && showComparison && (
            <View className="flex-row items-center">
              <Text
                className={`text-xs ${getImprovementIndicator(currentPlan.fatLoss, prediction.fatLoss, true).color}`}
              >
                {
        ...
        <View className="flex-row items-center">
          ...
          {isNew && showComparison && (
            <View className="flex-row items-center">
              <Text
                className={`text-xs ${getImprovementIndicator(currentPlan.muscleGain, prediction.muscleGain, true).color}`}
              >
                {
                  getImprovementIndicator(
                    currentPlan.muscleGain,
                    prediction.muscleGain,
                    true,
                  ).icon
                }
              </Text>
      ...
      {isCalculating && isNew && (
        <View className="absolute inset-0 bg-white bg-opacity-70 rounded-xl items-center justify-center">
          <Text className="text-blue-500 text-sm animate-pulse">
            Calculating...
          </Text>
        </View>
      )}
        ...
        <View className="bg-gray-50 p-4 rounded-xl">
          ...
          <View className="space-y-1">
            ...
            <View className="flex-row items-center justify-between">
              ...
              >
                {newPlan.fatLoss > currentPlan.fatLoss ? "+" : ""}
                {Math.round((newPlan.fatLoss - currentPlan.fatLoss) * 10) /
                  10}{" "}
                lbs
              </Text>
            </View>
            ...
            <View className="flex-row items-center justify-between">
              ...
              >
                {newPlan.muscleGain > currentPlan.muscleGain ? "+" : ""}
                {Number(newPlan.muscleGain - currentPlan.muscleGain).toFixed(
                  2,
                )}{" "}
                lbs
              </Text>
