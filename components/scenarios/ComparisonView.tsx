        ...
        <View className="bg-white rounded-lg p-6 mb-6">
          ...
          {selectedScenarios.length > 0 && (
            <View className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
              <Text className="text-green-800 font-semibold mb-2">🏆 Recommended Scenario</Text>
              {(() => {
                // Find the best scenario based on multiple factors
                const bestScenario = selectedScenarios.reduce((best, current) => {
                  const bestScore = (best.prediction.confidence * 0.4) +
                                   ((100 - best.prediction.timeline) * 0.3) +
                                   (best.prediction.fatLoss * 0.3);
                  const currentScore = (current.prediction.confidence * 0.4) +
                ...
                const getReason = (scenario: typeof bestScenario) => {
                  if (scenario.prediction.confidence >= 85) return "Highest confidence score with proven results";
                  if (scenario.prediction.timeline <= 8) return "Fastest timeline to reach your goals";
                  if (scenario.prediction.fatLoss >= 10) return "Maximum fat loss potential";
                  return "Best balance of timeline, confidence, and results";
                };

                return (
                  <View>
                    <Text className="text-green-700 font-medium">{bestScenario.name}</Text>
                    <Text className="text-green-600 text-sm mt-1">{getReason(bestScenario)}</Text>
                  </View>
                );
              })()}
            </View>
                ...
                return (
                  <View
                    ...
                    className={`mr-6 min-w-[220px] p-4 rounded-lg ${
                      isRecommended ? 'bg-green-100 border-2 border-green-300' : 'bg-white border border-gray-200'
                    }`}
                  >
                    <View className="flex-row items-center justify-between mb-3">
                      <Text className={`font-semibold ${isRecommended ? 'text-green-800' : 'text-gray-900'}`}>
                        {scenario.name}
                      </Text>
                      {isRecommended && (
                    ...
                    <View className="space-y-3">
                      ...
                      <View className="border-b border-gray-200 pb-2">
                        <Text className="text-xs font-semibold text-gray-500 uppercase mb-2">Results</Text>
                        <View className="space-y-1">
                          <View className="flex-row justify-between">
                            <Text className="text-sm text-gray-600">Body Fat Goal:</Text>
                            <Text className="text-sm font-semibold text-blue-600">{Number(scenario.prediction.targetBodyFat).toFixed(1)}%</Text>
                          </View>
                          <View className="flex-row justify-between">
                            <Text className="text-sm text-gray-600">Fat Loss:</Text>
                            <Text className="text-sm font-semibold text-red-600">{Number(scenario.prediction.fatLoss).toFixed(1)} lbs</Text>
                          </View>
                          <View className="flex-row justify-between">
                            <Text className="text-sm text-gray-600">Muscle Gain:</Text>
                            <Text className="text-sm font-semibold text-green-600">{Number(scenario.prediction.muscleGain).toFixed(1)} lbs</Text>
                          </View>
                        </View>
                      </View>
                      ...
                      <View className="border-b border-gray-200 pb-2">
                        ...
                        <View className="space-y-1">
                          <View className="flex-row justify-between">
                            ...
                            <Text className="text-sm font-semibold text-purple-600">{scenario.prediction.timeline} weeks</Text>
                          </View>
                          <View className="flex-row justify-between">
                            <Text className="text-sm text-gray-600">Confidence:</Text>
                            <Text className={`text-sm font-semibold ${
                              scenario.prediction.confidence >= 80 ? 'text-green-600' :
                              scenario.prediction.confidence >= 60 ? 'text-yellow-600' : 'text-red-600'
                            }`}>
                              {Number(scenario.prediction.confidence).toFixed(1)}%
                            </Text>
                          </View>
                        </View>
                      </View>
                      ...
                      {/* Parameters Section */}
                      <View>
                        <Text className="text-xs font-semibold text-gray-500 uppercase mb-2">Plan Details</Text>
                        <View className="space-y-1">
                          <View className="flex-row justify-between">
