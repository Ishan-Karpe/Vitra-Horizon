...
export const ProgressProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [bodyFatData, setBodyFatData] =
    useState<DataPoint[]>(generateBodyFatData);
  ...
  const getCurrentBodyFat = useCallback(() => {
    return bodyFatData.length > 0
      ? bodyFatData[bodyFatData.length - 1].value
      : userData.bodyFatPercentage || 25;
  ...
  const goalProgress: GoalProgress = useMemo(() => {
    ...
    const progress =
      totalChange !== 0 ? Math.round((achievedChange / totalChange) * 100) : 0;
  ...
  const getFilteredData = useCallback(
    (data: DataPoint[], period: TimePeriod): DataPoint[] => {
      const today = new Date();
      let daysBack = 7;
  ...
  const getBodyFatTrend = useCallback(
    () => getTrend(bodyFatData),
    [bodyFatData, getTrend],
  );
  ...
  const updateMeasurements = useCallback(
    (newMeasurements: Partial<MeasurementData>) => {
      setMeasurements((prev) => ({ ...prev, ...newMeasurements }));

      // If weight is being updated, propagate to UserDataContext and update weight data
      if (
        "weight" in newMeasurements &&
        typeof newMeasurements.weight === "number"
      ) {
      ...
      if (
        "bodyFatPercentage" in newMeasurements &&
        typeof newMeasurements.bodyFatPercentage === "number"
      ) {
